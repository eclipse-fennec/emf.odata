...
| `odata.persistence` | #13/#11 | Backend-neutrale Query+Write über die Fennec-Persistence-SPI (`CommandPersistenceService`: Reads via `OclToExpr`→`QueryableResource.query` mit Capability-Vorvalidierung, Writes via Insert/Update/DeleteCommand + ChangeSet-Templates; Nichtübersetzbares → 501) — bedient JPA UND Mongo, Ziel-Nachfolger von `odata.persistence.jpa` |
