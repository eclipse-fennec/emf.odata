...
// (guides.mjs) — internal dev docs (architecture change-log, backlogs, ADRs,
// conformance working notes) are deliberately NOT published. Cross-links inside
// the published docs that point at a NON-published file are rewritten to the
// GitHub blob URL so they keep working instead of 404-ing on the site; links
// between published docs become sibling routes.
//
// Links are resolved relative to each source file's directory (the manual lives
// in docs/manual/), so a bare/relative link resolves to the right target.
...
const blobBase = `https://github.com/eclipse-fennec/emf.odata/blob/${branch}/docs`;
